...
def check_user_policies(user_name):
    log_info("checking " + user_name + " policies")
    ...
    if 'AdministratorAccess' in policy_names:
        ...
        log_info(f"... NO AdministratorAccess ... {user_name} may have issue if you try to setup services ...fail");
...
def create_build(build_config): 
    ...
    match=re.search("Build ID: (.*)", completed_process.stdout.decode('utf-8'))
    if match:
...
def create_fleet(build_config):
    ...
    if build_id: 
        ...
        create_fleet_resp = gamelift_client.create_fleet(
            ...
            FleetType="ON_DEMAND",
            EC2InboundPermissions=[{'FromPort':7777,'ToPort':7777,'Protocol':'UDP','IpRange':'0.0.0.0/0'}]
        )
    else:
...
def create_lambda_role(role_name, assume_role_policy, other_policy_name, other_policy):
    try:
...
def create_lambda(function_name, role_arn, filename, replace_old=None, replace_new=None):
...
def create_lambdas(build_config):
  ...
  "Statement": [
    ...
    # ref: https://github.com/aws-samples/amazon-gamelift-unreal-engine/blob/main/lambda/GameLiftUnreal-StartGameLiftSession.json
    ...
    # todo: session_role = create_lambda_role(build_config["lambda_session_role"], lambda_policy, 
    session_role = create_lambda_role("gl_session_lambda_role", lambda_policy, 
                "gl_session_policy", gamelift_session_policy) 
    #todo: debug this login_role = create_lambda_role(build_config["lambda_cognito_role"], lambda_policy, 
    ...
    create_lambda(build_config["lambda_start_session_name"], session_role,"GameLiftUnreal-StartGameLiftSession.py",
            'GAMELIFT_FLEET_ID = ""', "GAMELIFT_FLEET_ID = \"" + fleet_id + "\"")

    
    cognito_app_client_id = lookup_cognito_pool_client_id(build_config["user_pool_name"], build_config["user_pool_login_client_name"]) 

    create_lambda(build_config["lambda_login_name"], login_role, "GameLiftUnreal-CognitoLogin.py", 
                        "USER_POOL_APP_CLIENT_ID = ''", "USER_POOL_APP_CLIENT_ID = \"" + cognito_app_client_id + "\"")
...
def create_rest_resource(
    ...
    try:
        ...
        apigateway_client.put_integration(
            restApiId=rest_api_id, resourceId=resource_id, httpMethod=http_method, type='AWS',
...
def create_login_resource(rest_api_id, build_config, authorizer_id):
    ...
    create_rest_resource(rest_api_id, apigateway_client, build_config["rest_api_login_path_part"], 'POST', account_id, 
            lambda_client,
            lambda_arn, 
            authorizer_id)
...
def create_start_session_resource(rest_api_id, build_config, authorizer_id):
    ...
    create_rest_resource(rest_api_id, apigateway_client, build_config["rest_api_start_session_path_part"], 'GET', account_id, 
            lambda_client,
            lambda_arn,
            authorizer_id)
...
def create_rest_api(build_config):
    ...
    #
    try:
        response = apigateway_client.create_rest_api(name=build_config["rest_api_name"])
        rest_api_id = response['id']
    except ClientError:
        logger.exception(f'Could not create REST API {build_config["rest_api_name"]}.')
        raise
    ...
    login_curl = 'curl -X POST -d "{\\"username\\":\\"user0\\", \\"password\\":\\"test12\\"}" ' + invoke_url + '/login'
    log_info(login_curl)
    log_info("")
    log_info('to start a session,  use the [IdToken] from the above login')
    start_curl = 'curl -X GET -H "Authorization: Bearer [IdToken]\" ' + invoke_url + '/startsession' 
    log_info(start_curl)
...
def create_user_pool(build_config):
    ...
    login_url=login_url + f'login?client_id={create_user_pool_client_resp["UserPoolClient"]["ClientId"]}'
    login_url=login_url + f'&response_type=Token&scope=email+openid&redirect_uri={redirect_uri}'
    log_info(login_url)
...
# walk through AWS services and query their states to see if they are present and
# plausibly functioning
def check_configured_services(profile, region, destructive, constructive, build_config):
    check_user_policies(profile)
...
def delete_user_pool(build_config):
    ...
    if pool_id:
        ...
        if "Domain" in response["UserPool"]:
            ...
            response = cognitoidp_client.delete_user_pool_domain(Domain=pool_domain, UserPoolId=pool_id)
        response = cognitoidp_client.delete_user_pool(UserPoolId=pool_id)

def delete_lambdas(build_config):
    log_info("deleting lambdas")
    start_session_arn = lookup_lambda_arn(build_config["lambda_start_session_name"])
...
def process_build_config(build_config):
    if len(build_config["commands"]) > 0:
        ...
        if len(sub_commands) > 0 and sub_commands[0] == "all":
            sub_commands = ["build", "fleet", "user_pool", "lambdas", "rest_api"]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Configure AWS Services to provide login, session and server management for dedicated UE servers',
             formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('commands', nargs='*')
    parser.add_argument('--prefix', default="test1", help="prefix used below")

    parser.add_argument('--build_name', default="[prefix]-build", help="name associated with the build (visible in the GameLift console")
    parser.add_argument('--build_version', default="build0.42", help="a version number")
    parser.add_argument('--build_os', default="WINDOWS_2012",  help = "the os to install on the EC2 instances")
    parser.add_argument('--build_root', default="E:/unreal_projects/MyProject/x64 Builds/WindowsServer",
                                    help="path to the server package on your local machine")


    parser.add_argument('--fleet_name', default="[prefix]-fleet", help="name associated with the fleet")
    parser.add_argument('--fleet_launch_path', default="C:/game/MyProject/Binaries/Win64/MyProjectServer.exe",
            help="the EC2 path to the server.  Must start with c:/game" )

    parser.add_argument('--fleet_ec2_instance_type', default="c5.large", help="what kind of EC2s to allocate.")

    parser.add_argument('--user_pool_name', default="[prefix]-user-pool", help="pool name")
    parser.add_argument('--user_pool_login_client_name', default="[prefix]-user-pool-login-client", help="pool client name")
    parser.add_argument('--user_pool_subdomain_prefix', default="[prefix]-login", help="name the subdomain")

    parser.add_argument('--lambda_start_session_name', default="[prefix]-lambda-start-session", help="name the lambda")
    parser.add_argument('--lambda_login_name', default="[prefix]-lambda-login", help="name the lambda")
    parser.add_argument('--lambda_session_role', default="[prefix]-lambda-session-role", help="name the role")
    parser.add_argument('--lambda_cognito_role', default="[prefix]-lambda-cognito-role", help="name the role")

    parser.add_argument('--rest_api_name', default="[prefix]-rest-api", help="name the api")
    parser.add_argument('--rest_api_stage_name', default="[prefix]-api-test-stage", help="name the stage")
    parser.add_argument('--rest_api_login_path_part', default="login", help="name the suffix")
    parser.add_argument('--rest_api_start_session_path_part', default="startsession", help="name the suffix")
    parser.add_argument('--rest_api_cognito_authorizer_name', default="[prefix]-cognito-authorizer", help="name the authorizer")

    parser.add_argument('--profile', default='sean_gl', help="AWS credentials to use")
